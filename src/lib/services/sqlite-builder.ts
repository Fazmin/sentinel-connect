...
export class SQLiteBuilder {
  ...
  addMetadata(metadata: Record<string, string>): void {
    ...
    this.db.exec(`
      CREATE TABLE IF NOT EXISTS _sentinel_metadata (
        key TEXT PRIMARY KEY,
        value TEXT
      )
    `);

    const stmt = this.db.prepare('INSERT OR REPLACE INTO _sentinel_metadata (key, value) VALUES (?, ?)');
    ...
    stmt.run('sentinel_version', '1.0.0');
  }
  ...
  private async encryptFile(inputPath: string, outputPath: string, key: string): Promise<void> {
    ...
    const derivedKey = crypto.scryptSync(key, 'sentinel-file-salt', 32);
