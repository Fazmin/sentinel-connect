...
export async function sendOtpEmail(email: string, otpCode: string, userName?: string): Promise<{ success: boolean; error?: string }> {
  ...
  const subject = 'Your SentinelConnect Login Code';
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f4f4f5; margin: 0; padding: 40px 20px;">
  <div style="max-width: 480px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
    ...
    <div style="background: linear-gradient(135deg, #10b981 0%, #14b8a6 100%); padding: 32px; text-align: center;">
      <h1 style="color: #ffffff; margin: 0; font-size: 24px; font-weight: 700;">SentinelConnect</h1>
      <p style="color: rgba(255,255,255,0.9); margin: 8px 0 0; font-size: 14px;">Secure Data Provisioning</p>
    </div>
    ...
    <div style="background-color: #f9fafb; padding: 20px 32px; border-top: 1px solid #e5e7eb;">
      <p style="color: #9ca3af; font-size: 12px; margin: 0; text-align: center;">
        This is an automated message from SentinelConnect.<br>
...
export async function sendApprovalEmail(email: string, userName?: string): Promise<{ success: boolean; error?: string }> {
  ...
  const subject = 'Your SentinelConnect Account Has Been Approved';
  
  const text = `
${greeting},

Great news! Your SentinelConnect account has been approved.
  ...
  <div style="max-width: 480px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
    ...
    <div style="padding: 32px;">
      ...
      <p style="color: #374151; font-size: 16px; margin: 0 0 24px;">
        You can now log in to SentinelConnect using your email address. Each time you log in, you'll receive a one-time verification code via email.
      </p>
